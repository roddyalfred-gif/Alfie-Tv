...
class AlfiePlayer(context: Context) {
        ...
        .build().apply {
            ...
            addListener(object : Player.Listener {
                override fun onPlaybackStateChanged(playbackState: Int) {
                    ...
                    } else if (playbackState == Player.STATE_READY) bufferingSince = 0L
                }
    ...
    private val mediaSession: MediaSession by lazy {
        MediaSession.Builder(appContext, player).setId("alfie-tv").build()
    }
    init { mediaSession }
    ...
    fun play(url: String, title: String? = null, positionMs: Long = C.TIME_UNSET, channelNumber: String? = null) {
        ...
        currentUrl = url; currentTitle = title; currentChannelNumber = channelNumber
        recoveryAttempts = 0; audioRecoveryAttempts = 0; recovering = false
        bufferingSince = 0L; stagnantSince = 0L; lastPlayingPosition = C.TIME_UNSET
        startupStartedAt = System.currentTimeMillis()
        diagnostics.startupLatencyMs = null; diagnostics.bitrate = null; diagnostics.resolution = null
        diagnostics.bufferedSeconds = null; diagnostics.lastErrorCategory = null; diagnostics.lastErrorCode = null
        ...
        val builder = MediaItem.Builder().setUri(url).setMediaId(title ?: "alfie-tv").setMediaMetadata(
    ...
    fun switchChannel(url: String, title: String? = null, channelNumber: String? = null) {
        ...
        pendingChannelUrl = url; pendingChannelTitle = title; pendingChannelNumber = channelNumber
    ...
    fun stop() {
        playbackGeneration.next(); handler.removeCallbacks(channelSwitchRunnable)
        pendingChannelUrl = null; pendingChannelTitle = null; pendingChannelNumber = null
        lastPositionMs = player.currentPosition.coerceAtLeast(0L); player.stop()
    }

    fun release() {
        playbackGeneration.next(); handler.removeCallbacks(healthCheck); handler.removeCallbacks(channelSwitchRunnable)
        pendingChannelUrl = null; pendingChannelTitle = null; pendingChannelNumber = null
        lastPositionMs = player.currentPosition.coerceAtLeast(0L); currentUrl = null
    ...
    fun refreshAudio() { if (player.currentMediaItem == null || recovering) return; recoverAudio(force = true) }
    ...
    fun statusText(): String = when {
        ...
        player.playerError != null -> "ERROR ${diagnostics.lastErrorCategory ?: "unknown"}"
    ...
    private fun checkPlaybackHealth() {
        ...
        updateVideoDiagnostics()
        if (startupStartedAt != 0L && !diagnostics.firstFrameRendered && now - startupStartedAt >= 15_000L) { if (autoRetryEnabled) recover(); return }
        if (p.playbackState == Player.STATE_ENDED && p.isCurrentMediaItemLive) { if (autoRetryEnabled) recover(); return }
        ...
        if (videoPlaying && audioAvailable) {
            ...
            if (lastPlayingPosition != C.TIME_UNSET && position == lastPlayingPosition) {
                ...
                if (autoRetryEnabled && AudioRecoveryPolicy.shouldRecover(videoPlaying, audioAvailable, now - stagnantSince, now, lastAudioRecoveryAt, audioRecoveryAttempts)) recoverAudio()
    ...
    private fun recoverAudio(force: Boolean = false) {
        ...
        if (!AudioRecoveryPolicy.shouldRecover(player.isPlaying || player.playWhenReady, diagnostics.audioTrackAvailable, positionStagnantForMs, now, lastAudioRecoveryAt, audioRecoveryAttempts, force)) return
        ...
        val position = player.currentPosition.coerceAtLeast(0L)
        stagnantSince = 0L; lastPlayingPosition = C.TIME_UNSET; diagnostics.audioTrackAvailable = false
        player.stop()
        if (live) { player.setMediaItem(item); player.prepare(); player.seekToDefaultPosition() }
        else { player.setMediaItem(item, position); player.prepare(); player.seekTo(position) }
    ...
    private fun recover() {
        ...
        bufferingSince = 0L; stagnantSince = 0L; lastPlayingPosition = C.TIME_UNSET; startupStartedAt = now
        diagnostics.firstFrameRendered = false; diagnostics.audioTrackAvailable = false; diagnostics.videoTrackAvailable = false
        val generation = playbackGeneration.current(); val live = player.isCurrentMediaItemLive
        val position = player.currentPosition.coerceAtLeast(0L); val originalItem = player.currentMediaItem
        handler.postDelayed({
    ...
    private fun inferSourceType(url: String): SourceType {
        val path = url.substringBefore('?').lowercase(); val query = url.substringAfter('?', "").lowercase()
        return when {
